...
// voter should be able to vote only if registered
rule onlyRegisteredVoterCanVote(method f, address voter, address first, address second, address third) {
	env e;
	
	uint256 age;
	bool voterReg;
	bool voterRegReturned;
	bool voted;
	uint256 vote_attempts;
	bool black_listed;
	age, voterReg, voted, vote_attempts, black_listed = getFullVoterDetails(e, voter);
	
	require(e.msg.sender == voter);
	voterRegReturned = vote(e, first, second, third);
	
	assert(voterReg == voterRegReturned, "voter reg status and vote() status do not match");
}