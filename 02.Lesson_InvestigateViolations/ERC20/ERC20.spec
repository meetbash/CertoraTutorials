...
// the state starts with the total sum of all user balances being greater than totalSupply. This is why this rule fails, 
// no users alone has a balance which is greater than supply but 2 ussers together do, so burning/transfering tokens can break the rule
rule totalSupplyNotLessThanSingleUserBalance(method f, address user) {
    ...
    // kind of hacky but if calling these functions, then the accounts combined supply should be less than 
    // total supply before calling the function. This will tell us if the function call is broken or just our rule.
    require (f.selector == transfer(address, uint256).selector ||
         f.selector == transferFrom(address, address, uint256).selector ||
         f.selector == burn(address, uint256).selector) => 
         (balanceOf(e, args) + userBalanceBefore) < totalSupplyBefore && 
         (balanceOf(e, e.msg.sender) + userBalanceBefore) < totalSupplyBefore;
    f(e, args);
